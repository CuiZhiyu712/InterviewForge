...
export const manualAssignments=[
  ...
  ["src-1103","P0",["Agent"],["Agent"],"求职定位与项目叙事"],
  ["src-0423","P0",["系统设计"],["秒杀"],"Java 后端实习与秒杀链路"],
  ["src-1162","P0",["RAG"],["RAG"],"RAG 全链路与质量评测"],
  ["src-1121","P0",["Agent"],["Agent","Workflow"],"Agent 与 Multi-Agent 取舍"],
  ...
  ["src-1146","P0",["LLM工程"],["幻觉"],"大模型应用故障与安全"],
  ["src-0763","P1",["Java基础"],["线程池"],"Java 并发与流式接口"],
  ["src-0768","P1",["Java基础"],["synchronized","lock"],"Java 并发与流式接口"],
  ["src-0869","P1",["MySQL"],["MVCC"],"MySQL 数据建模与事务"],
  ["src-0937","P1",["MySQL"],["MVCC"],"MySQL 数据建模与事务"],
  ["src-0463","P1",["消息队列"],["RocketMQ","重复消费"],"Redis 与消息最终一致性"],
  ["src-1188","P1",["LLM工具调用"],["SSE"],"SSE 协议与异常收尾"],
  ["src-1171","P0",["RAG"],["量化","RAG"],"RAG 全链路与质量评测"],
  ...
  ["src-1195","P1",["LLM工具调用"],["MCP"],"Tool Calling 与 MCP 安全"],
  ["src-1179","P1",["RAG"],["Embedding","RAG"],"Milvus 与向量数据生命周期"],
  ["src-1183","P1",["LLM工具调用"],["Function Calling"],"Tool Calling 与 MCP 安全"],
].map(([sourceId,priority,modules,requiredAny,topic])=>({sourceId,priority,modules,requiredAny,topic}));
...
const strictRules=[
  ...
  {topic:"大模型应用故障与安全",modules:["LLM工程"],any:["幻觉","prompt","限流","重试","降级","api key"],cap:7},
...
export function mapResumePriorities(databasePath,focusPath){requireFile(databasePath,"database");requireFile(focusPath,"focus configuration");const config=JSON.parse(readFileSync(focusPath,"utf8"));validateFocus(config);const topics=new Map(config.priorityTopics.map(t=>[t.module,t]));const db=new DatabaseSync(databasePath);try{db.exec("PRAGMA foreign_keys=ON");const rows=db.prepare(`SELECT q.id,q.source_id,q.question,q.original_title,q.module,q.submodule,CASE WHEN a.id IS NULL THEN 0 ELSE 1 END has_answer FROM questions q LEFT JOIN answers a ON a.question_id=q.id ORDER BY q.id`).all().map(r=>({...r,text:normalize(`${r.question} ${r.original_title}`)}));const bySource=new Map(rows.map(r=>[r.source_id,r])),assignments=new Map(),usedTitleKeys=new Set();
    ...
    if(assignments.size===0)throw Error("no trustworthy priority mappings");const manualLegacy=new Set(manualAssignments.map(x=>x.sourceId));const unmapped=legacyQuestions.filter(q=>q.resumeFocus&&!manualLegacy.has(q.sourceId)).map(q=>q.id);
    const clear=db.prepare("UPDATE questions SET resume_focus=0,priority=NULL,resume_reason=''");const apply=db.prepare("UPDATE questions SET resume_focus=1,priority=?,resume_reason=? WHERE id=?");const insert=db.prepare("INSERT INTO answer_reviews(question_id,batch,status,notes,reviewed_at) VALUES(?,?,'pending',?,NULL)");db.exec("BEGIN IMMEDIATE");try{clear.run();db.prepare("UPDATE answers SET review_status='draft' WHERE question_id IN (SELECT question_id FROM answer_reviews WHERE batch IN ('resume-priority-v2','resume-priority-v3'))").run();db.prepare("DELETE FROM answer_reviews WHERE batch IN ('resume-priority-v2','resume-priority-v3')").run();for(const [id,a] of assignments){const reason=a.reasons.join("\n");apply.run(a.priority,reason,id);if(["P0","P1"].includes(a.priority))insert.run(id,reviewBatch,`待审关注点：核对答案是否准确覆盖“${reason.split("：")[0]}”所需机制、边界与项目真实性，不得把邻近能力冒充项目实作。`)}db.exec("COMMIT")}catch(e){db.exec("ROLLBACK");throw e}
...
const isMain=process.argv[1]&&pathToFileURL(resolve(process.argv[1])).href===import.meta.url;if(isMain){try{const o=parseArguments(process.argv.slice(2)),r=mapResumePriorities(o.databasePath,o.focusPath);console.log(`resume priorities mapped: ${JSON.stringify(r.counts)}`);console.log(`unmapped legacy IDs: ${r.unmappedLegacyIds.join(",")||"none"}`)}catch(e){console.error(e.message);process.exitCode=1}}
